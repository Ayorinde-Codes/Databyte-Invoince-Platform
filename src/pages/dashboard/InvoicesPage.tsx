...
} from '@/components/invoices/ValidationResultDialog';
import {
  Card,
  CardContent,
  CardDescription,
  ...
  const handleQuickFixUpdate = async () => {
    ...
    try {
      ...
      if (validationResult && fix.action === 'update_invoice_item' && fix.item_id) {
        const updatedQuickFixes =
          validationResult.quick_fixes?.filter(
            (qf: QuickFix) => !(qf.item_id === fix.item_id && qf.type === fix.type)
          ) || [];
        ...
        const remainingRequired = updatedQuickFixes.filter((qf) => qf.blocking !== false && (
          qf.blocking === true ||
          ['item_hsn_code', 'item_isic_code', 'item_uom', 'party_tin', 'firs_invoice_type_code', 'firs_note', 'previous_invoice_irn'].includes(qf.type)
        ));
        ...
        // Warnings/recommended items do not need to be cleared to continue
        if (updatedErrors.length === 0 && remainingRequired.length === 0) {
  ...
  return (
    <DashboardLayout>
      ...
      <ValidationResultDialog
        ...
        partyLinkId={validationPartyLinkId}
